...
var API = class {
  ...
  async getData(url) {
    ...
    if (parsedUrl.type === "track") {
      ...
      return api.getTrack(id).then(({ body }) => body).catch((e) => {
        throw apiError(e);
      });
    }
    if (!this._tokenAvailable) {
    ...
    try {
      switch (parsedUrl.type) {
        ...
        case "playlist": {
          ...
          thumbnail = body.images?.[0]?.url;
          url = body.external_urls?.spotify;
          tracks = (await __privateMethod(this, _getFullItems, getFullItems_fn).call(this, body)).map((i) => i.track);
          break;
        }
        case "artist": {
...
var SpotifyPlugin = class extends import_distube2.CustomPlugin {
  constructor(options = {}) {
    super();
    ...
    (0, import_distube2.checkInvalidKey)(options, ["parallel", "emitEventsAfterFetching", "api", "songsPerRequest", "requestDelay"], "SpotifyPluginOptions");
  ...
  async play(voiceChannel, url, options) {
    ...
    if (data.type === "track") {
      ...
      const getFirstSong = /* @__PURE__ */ __name(async () => {
        ...
        if (!result)
          return;
        firstSong = new import_distube2.Song(result, { member, metadata });
      }, "getFirstSong");
      while (!firstSong)
        await getFirstSong();
      if (!firstSong) {
      ...
      const fetchTheRest = /* @__PURE__ */ __name(async (q, fs) => {
        if (queries.length) {
          ...
          if (this.parallel) {
            ...
            let Global = this;
            await Promise.map(queries, async function (query) {
              let search_result = await Global.search(query);
              results.push(search_result);
              if (!search_result) return;
              const s = new import_distube2.Song(search_result, { member, metadata });
              s.playlist = playlist; // Maintain playlist association
              playlist.songs.push(s); // Add to playlist
              q.addToQueue(s, !skip && position > 0 ? position + 1 : position); // Add to queue

              return Promise.delay(Global.requestDelay);
              //return Promise.delay(Global.requestDelay);
            }, { concurrency: Global.songsPerRequest });
          } else {
            for (let i = 0; i < queries.length; i++) {
              let search_result = await this.search(queries[i]);
              results.push(search_result);
              if (!search_result) return;
              const s = new import_distube2.Song(search_result, { member, metadata });
              s.playlist = playlist; // Maintain playlist association
