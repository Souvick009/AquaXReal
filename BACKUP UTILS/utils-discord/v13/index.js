...
module.exports = class DiscordUtility {
    ...
    static async createEmbedPages(client, message, array, options = {
        ...
        if (title) embed.setTitle(title)
        if (thumbnail) embed.setThumbnail(thumbnail)
        embed.setFooter({text: `${footer} | Page: ${pageno}/${Math.ceil(newArray.length / perpage)}`, iconURL: footerImage})
        if (timestamp) embed.setTimestamp();
        if (author) embed.setAuthor({
            name: author,
            iconURL: authorImage
        ...
        let back = new Discord.ButtonBuilder()
            .setCustomId("back")
            .setLabel(buttonPrev)
        ...
        let ending = new Discord.ButtonBuilder()
            .setCustomId("ending")
            .setLabel(buttonLast)
            .setStyle(Discord.ButtonStyle.Primary);
        var deleteEmbed = new Discord.ButtonBuilder()
        ...
        if (isUserlogs && array.timeouts.length !== 0) {
            mutesortimeouts = new Discord.ButtonBuilder()
                .setCustomId("change")
                .setLabel("Show Timeouts")
                .setStyle(Discord.ButtonStyle.Primary);
            let row1 = new Discord.ActionRowBuilder().addComponents([starting, back, next, ending, deleteEmbed])
            let row = new Discord.ActionRowBuilder().addComponents([mutesortimeouts])
            comps = [row1, row]
        } else {
            comps = [new Discord.ActionRowBuilder().addComponents([starting, back, next, ending, deleteEmbed])]
        ...
        if (newArray.length < perpage && isUserlogs) {
            if (array.timeouts.length !== 0) {
                mutesortimeouts = new Discord.ButtonBuilder()
                    ...
                    .setLabel("Show Timeouts")
                    .setStyle(Discord.ButtonStyle.Primary);
                comps = [new Discord.ActionRowBuilder().addComponents([mutesortimeouts])]
            }
