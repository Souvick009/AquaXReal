...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        var createBar = function (maxtime, currenttime, size = 15, line = "▬", slider = "🔶") {
           let bar = currenttime > maxtime ? [line.repeat(size / 2 * 2), (currenttime / maxtime) * 10] : [line.repeat(Math.round(size / 2 * (currenttime / maxtime))).replace(/.$/, slider) + line.repeat(size - Math.round(size * (currenttime / maxtime)) + 1), currenttime / maxtime];
