...
module.exports = {
    ...
    run: async (bot, message, args, options, author) => {
        ...
        let results;
        try {
            results = await ytsr(req, { limit: 10 })

            if (!fetch.items.length) {
                const embed = new Discord.EmbedBuilder()
                embed.setColor("#FF0000");
                embed.setDescription(`:x: No result found for ${req}!`)
        ...
        for (i = 0; i < 10; i++) {
            try {
                searchResult += `**${i + 1})** [${results.items[i].name}](${results.items[1].url}) - \`${results.items[i].duration ? results.items[i].duration : "🔴 LIVE"}\`\n`
            }
            catch (error) {
                searchResult = "\n";
            }
        ...
        const embed = new Discord.EmbedBuilder()
        embed.setTitle(`SearchResults for: ${req}`.substring(0, 256))
        ...
        collector.on("collect", async (interaction) => {
            ...
            try {
                bot.distube.play(interaction.member.voice.channel, results.items[userinput - 1].url, {
