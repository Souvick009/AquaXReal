...
module.exports = class DiscordUtility {
    ...
    static async reactMessage(message, user, emoji, fallbackEmoji = "✅") {
        ...
        if (fallbackEmoji && (!dm && !message.channel.permissionsFor(user).has(Discord.PermissionFlagsBits.UserExternalEmojis))) {
            emoji = fallbackEmoji;
        }
        if (dm || message.channel.permissionsFor(user).has([Discord.PermissionFlagsBits.AddReactions, Discord.PermissionFlagsBits.ReadMessageHistory])) {
            try {
    ...
    static async awaitPlayers(msg, join, emoji, max, min = 1) {
        ...
        await send(message, { content: `You will need at least ${min - 1} more player(s) (at max ${max - 1}). To join, type \`${join}\`.` });
        const joined = [];
    ...
    static async createEmbedPages(client, message, array, options = { emojis: {} }) {
        ...
        let stoppage = new Discord.ButtonBuilder()
            .setCustomId(id5)
            .setLabel(buttonEnd)
        ...
        if (array.length < perpage && !buttons) {
            msg = await send(message, { embeds: [embed] });
        } else if (array.length > perpage && buttons) {
            msg = await send(message, { components: [new Discord.ActionRowBuilder().addComponents([starting, back, next, ending, stoppage])], embeds: [embed] });
        } else {
            msg = await send(message, { embeds: [embed] });
        }
        ...
        if (array.length > perpage) {
            if (!buttons) {
                ...
                // when setting up the collector, capture the user properly
                const authorId = message.author?.id || message.user?.id || interaction.user.id; // fallback for safety
                collector.on("collect", async (button) => {
                    if (button.user.id !== authorId) return button.reply({ content: "This is not your interaction", ephemeral: true })
                    const reactionadd = array.slice(first + perpage, second + perpage).length;
                    const reactionremove = array.slice(first - perpage, second - perpage).length;
                    if (button.customId === id1) {
                        ...
                        embed.setDescription(`${header ? `${header}` : ""}${array.slice(first, second).join(joinBy)}${watermark ? `\n\n${watermark}` : ""}`);
                        embed.setFooter({ text: `${footer} | Page: ${pageno}/${Math.ceil(array.length / perpage)}`, iconURL: footerImage });
                        button.update({ components: [new Discord.ActionRowBuilder().addComponents([starting, back, next, ending, stoppage])], embeds: [embed] })
